            ...
            for cond in condtns:
                ...
                if cond['condition_type'] in category_list:
                    ...
                    for opt in cond['condition_option_category']:
                        opt_val = condition_category[opt]
                        r1.append(opt_val)
                    cla['options'] = {
                        "category": r1
                    }
...
class ModuleManager(object):
    ...
    def check_for_required_create_parameters(self):
        if self.want.name is None:
            raise F5ModuleError(
                "Creating SSLO config policy requires 'name'"
    ...
    def __init__(self):
        ...
        argument_spec = dict(
            ...
            policy_rules=dict(
                ...
                options=dict(
                    ...
                    conditions=dict(
                        ...
                        options=dict(
                            condition_type=dict(
                                choices=list(condition_type.keys())
                            ),
                            condition_option_category=dict(type='list', elements='str'),
            ...
            proxy_connect=dict(
                type='dict',
                options=dict(
                    pool_members=dict(type='list', elements='dict'),
                    pool_name=dict(),
                    username=dict(),
                    password=dict(
                        no_log=True
                ...
                # required_if=[
                #     ('pool_create', True, ['pool_members'], True)],
                mutually_exclusive=[
